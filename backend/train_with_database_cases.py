...
class EnhancedBNSClassifierV2:
    ...
    def train_models(self, df: pd.DataFrame):
        ...
        print(
            f"   Cross-validation: {cv_scores_type.mean():.1%} ± {cv_scores_type.std():.1%}"
        )
        print(f"\n🚨 Priority Accuracy: {priority_accuracy:.1%}")
        print(
            f"   Cross-validation: {cv_scores_priority.mean():.1%} ± {cv_scores_priority.std():.1%}"
        )
...
def main():
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        print(
            f"   📂 Case Type: {result['case_type']} ({result['case_type_confidence']:.1%})"
        )
        print(
            f"   🚨 Priority: {result['priority']} ({result['priority_confidence']:.1%})"
        )
        print(
            f"   ⚖️  BNS Section: {result['bns_section']} ({result['bns_confidence']:.1%})"
        )
