...
async def get_metrics(
    ...
    return {
        ...
        "summary": {
            ...
            "unplaced_percentage": round((unplaced_cases / total_cases) * 100, 1)
            if total_cases > 0
            else 0,
...
async def get_case_statistics(
    ...
    # Age distribution
    today = date.today()
    age_buckets = {"0-30_days": 0, "31-90_days": 0, "91-180_days": 0, "180+_days": 0}
    ...
    override_rate = (
        (len(overridden_cases) / total_cases) * 100 if total_cases > 0 else 0
    )
...
async def export_cause_list(
    ...
    if format == "csv":
        ...
        return _export_pdf(
            cause_list_data, f"Cause List - {date.strftime('%B %d, %Y')}"
        )
    ...
    if current_user.role in ["clerk", "admin"]:
        ...
        dashboard_data["quick_stats"] = {
            ...
            "classification_rate": round(
                ((my_cases - unclassified) / my_cases) * 100, 1
            )
            if my_cases > 0
            else 0,
        }
        ...
        today_hearings_stmt = select(func.count(Hearing.id)).where(
            Hearing.judge_id == current_user.id, Hearing.hearing_date == today
        )
