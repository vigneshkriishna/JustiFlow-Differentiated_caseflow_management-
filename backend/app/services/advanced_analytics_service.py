...
class AdvancedAnalyticsService:
    ...
    def get_real_time_metrics(self) -> Dict:
        ...
        recent_predictions = [
            p
            for p in self.analytics_data["predictions"]
        ...
        confidence_dist = {
            ...
            "medium": len(
                [p for p in recent_predictions if 0.6 <= p["confidence"] < 0.8]
            ),
        ...
        avg_processing_time = (
            sum([p["processing_time"] for p in recent_predictions])
        ...
        return {
            "overview": {
                ...
                "success_rate": round(
                    (confidence_dist["high"] + confidence_dist["medium"])
                    / len(recent_predictions)
                    * 100,
                    1,
                )
    ...
    def record_prediction(self, prediction_data: Dict):
        ...
        if len(self.analytics_data["predictions"]) > 1000:
            self.analytics_data["predictions"] = self.analytics_data["predictions"][
                -1000:
            ]
        ...
        report = {
            "summary": {
                ...
                "average_confidence": round(
                    sum([p["confidence"] for p in week_predictions])
                    / len(week_predictions),
                    3,
                )
                if week_predictions
                else 0,
                "high_confidence_rate": round(
                    len([p for p in week_predictions if p["confidence"] >= 0.8])
                    / len(week_predictions)
                    * 100,
                    1,
                )
    ...
    def _analyze_by_case_type(self, predictions: List[Dict]) -> Dict:
        ...
        for case_type, confidences in case_types.items():
            analysis[case_type] = {
                ...
                "high_confidence_rate": round(
                    len([c for c in confidences if c >= 0.8]) / len(confidences) * 100,
                    1,
                ),
            }
    ...
    def _analyze_by_complexity(self, predictions: List[Dict]) -> Dict:
        ...
        for complexity, confidences in complexity_levels.items():
            analysis[complexity] = {
                ...
                "average_confidence": round(sum(confidences) / len(confidences), 3),
                "success_rate": round(
                    len([c for c in confidences if c >= 0.7]) / len(confidences) * 100,
                    1,
                ),
            }
    ...
    def _analyze_by_confidence(self, predictions: List[Dict]) -> Dict:
        ...
        return {
            "high_confidence": {
                "count": len(high_conf),
                "percentage": round(len(high_conf) / len(predictions) * 100, 1)
                if predictions
                else 0,
            ...
            "medium_confidence": {
                ...
                "percentage": round(len(medium_conf) / len(predictions) * 100, 1)
                if predictions
                else 0,
                "avg_processing_time": round(
                    sum([p["processing_time"] for p in medium_conf]) / len(medium_conf),
                    2,
                )
                if medium_conf
                else 0,
            ...
            "low_confidence": {
                "count": len(low_conf),
                "percentage": round(len(low_conf) / len(predictions) * 100, 1)
                if predictions
                else 0,
                "avg_processing_time": round(
