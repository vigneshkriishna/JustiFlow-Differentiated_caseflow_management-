...
class AnalyticsService:
    ...
    def get_dashboard_overview(
        ...
        return {
            ...
            "bns_analytics": {
                "top_sections": dict(
                    sorted(bns_sections.items(), key=lambda x: x[1], reverse=True)[:10]
                ),
            ...
            "user_analytics": {
                ...
                "new_users_this_week": len(
                    [u for u in total_users if u.created_at >= seven_days_ago]
                ),
            },
            "performance_metrics": {
                "average_case_processing_time": "12.5 days",  # Mock - calculate from actual data
                "cases_resolved_this_month": len(
                    [c for c in total_cases if c.status == CaseStatus.DISPOSED]
                ),
                "pending_cases": len(
                    [
                        c
                        for c in total_cases
                        if c.status in [CaseStatus.FILED, CaseStatus.UNDER_REVIEW]
                    ]
                ),
    ...
    def get_case_analytics(self, session: Session, days: int = 30) -> Dict[str, Any]:
        ...
        complexity_distribution = {
            "simple": len(
                [c for c in cases if self._get_case_complexity(c) == "simple"]
            ),
            "medium": len(
                [c for c in cases if self._get_case_complexity(c) == "medium"]
            ),
            "complex": len(
                [c for c in cases if self._get_case_complexity(c) == "complex"]
            ),
        }
        ...
        return {
            ...
            "success_metrics": {
                ...
                "escalated_cases": len(
                    [c for c in cases if c.priority == CasePriority.URGENT]
                ),
            },
        }
    ...
    def get_bns_classification_analytics(self, session: Session) -> Dict[str, Any]:
        ...
        section_categories = {
            "violent_crimes": len(
                [
                    s
                    for s in section_frequency.keys()
                    if s in ["326", "302", "307", "323"]
                ]
            ),
            "property_crimes": len(
                [
        ...
        return {
            "classification_overview": {
                ...
                "classification_rate": round(
                    classification_success / len(cases) * 100, 1
                )
                if cases
                else 0,
            ...
            "section_frequency": dict(
                sorted(section_frequency.items(), key=lambda x: x[1], reverse=True)
            ),
            ...
            "model_performance": {
                ...
                "medium_confidence_predictions": len(
                    [c for c in confidence_scores if 0.6 <= c <= 0.8]
                ),
                "low_confidence_predictions": len(
                    [c for c in confidence_scores if c < 0.6]
                ),
    ...
    def get_user_activity_analytics(self, session: Session) -> Dict[str, Any]:
        ...
        return {
            ...
            "user_engagement": {
                "highly_active_users": len(
                    [
                        uid
                        for uid in active_users
                        if len([log for log in recent_logs if log.user_id == uid]) > 50
                    ]
                ),
                "moderately_active_users": len(
                    [
                        uid
                        for uid in active_users
                        if 10
                        <= len([log for log in recent_logs if log.user_id == uid])
                        <= 50
                    ]
                ),
                "low_activity_users": len(
                    [
                        uid
                        for uid in active_users
                        if len([log for log in recent_logs if log.user_id == uid]) < 10
                    ]
                ),
            },
        }
    ...
    def get_court_schedule_analytics(self, session: Session) -> Dict[str, Any]:
        ...
        timing_analysis = {
            "morning_slots": len(
                [
                    ...
                    for h in hearings
                    if 9
                    <= (h.scheduled_time.hour if hasattr(h, "scheduled_time") else 10)
                    < 12
                ]
            ),
            "afternoon_slots": len(
                [
                    ...
                    for h in hearings
                    if 12
                    <= (h.scheduled_time.hour if hasattr(h, "scheduled_time") else 14)
                    < 17
                ]
            ),
        ...
        return {
            "schedule_overview": {
                ...
                "average_hearings_per_day": round(
                    len(hearings) / 30, 1
                ),  # Mock: last 30 days
